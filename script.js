...
/* TikGrab 4.8 — local rating & reviews */
(function initLocalReviews(){
 const K='tikgrab_reviews_v48', stars=[...document.querySelectorAll('#ratingStars [data-rating]')];
 ...
 if(!stars.length||!submit||!list)return;
 let rating=0;
 const load=()=>{try{const x=JSON.parse(localStorage.getItem(K)||'[]');return Array.isArray(x)?x:[]}catch(e){return[]}};
 const save=x=>localStorage.setItem(K,JSON.stringify(x));
 const esc=x=>String(x).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
 const starsText=r=>'★'.repeat(r)+'☆'.repeat(5-r);
 function render(){
   const data=load(), total=data.reduce((s,x)=>s+x.rating,0);
   avg.textContent=`${data.length?(total/data.length).toFixed(1):'0.0'} / 5`;
   count.textContent=data.length?`${data.length} ulasan`:'Belum ada ulasan';
   list.innerHTML=''; empty.style.display=data.length?'none':'block';
   data.slice().reverse().forEach(x=>{
     const c=document.createElement('div'); c.className='tg48-review-card border border-gray-100 rounded-xl p-4 bg-white';
     c.innerHTML=`<div class="flex items-start justify-between gap-3"><div><div class="font-semibold text-gray-900">${esc(x.name||'Pengguna TikGrab')}</div><div class="text-yellow-400 text-sm mt-0.5">${starsText(x.rating)}</div></div><div class="text-[11px] text-gray-400">${esc(x.date)}</div></div><p class="text-sm text-gray-600 mt-3 leading-6">${esc(x.text)}</p>`;
     list.appendChild(c);
   });
 }
 ...
 submit.addEventListener('click',()=>{
   ...
   if(!rating)return; if(!t){status.textContent='Tulis ulasan terlebih dahulu.';text.focus();return}
   const data=load(); data.push({rating,name:n||'Pengguna TikGrab',text:t.slice(0,300),date:new Date().toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'})});
   save(data); rating=0; name.value=''; text.value=''; update(); status.textContent='Ulasan tersimpan di perangkat ini. Terima kasih! ❤️'; render();
 });
 update(); render();
})();
