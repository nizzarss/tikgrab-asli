...
/* TikGrab 5.2 — compact public reviews */
(function initCompactReviews(){
  const toggle = document.getElementById('reviewsCompactToggle');
  const extra = document.querySelector('.tg-rating-extra');
  if (toggle && extra) {
    toggle.addEventListener('click', () => {
      const open = extra.classList.toggle('tg-open');
      toggle.textContent = open ? '🙈 Sembunyikan ulasan' : '💬 Lihat ulasan';
    });
  }
  const formBtn = document.getElementById('openReviewForm');
  if (formBtn) {
    formBtn.addEventListener('click', () => {
      const target = document.querySelector('#reviewForm, #ratingForm, [data-review-form]');
