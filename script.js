...
async function downloadPack() {
    ...
    if (typeof JSZip === "undefined") {
        showToast("Komponen ZIP belum siap. Muat ulang halaman lalu coba lagi.", "error");
        return;
    }

    const files = [];
    const safeTitle = String(currentVideo.title || "TikGrab Video")
        .replace(/[\\/:*?"<>|\x00-\x1F]/g, " ")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 80) || "TikGrab Video";

    const videoUrl = currentVideo.play || "";
    const audioUrl = currentVideo.music || "";
    const coverUrl = currentVideo.cover || currentVideo.origin_cover || currentVideo.ai_dynamic_cover || "";

    if (!videoUrl) {
        showToast("Video tidak tersedia untuk Download Pack.", "error");
        return;
    }

    showToast("Menyiapkan Download Pack...", "info");
    setDownloadStatus(5, "Menyiapkan paket...");

    async function fetchBlob(url, label) {
        if (!url || !/^https?:\/\//i.test(url)) throw new Error(`${label} tidak tersedia`);
        const response = await fetch(url, { method: "GET", mode: "cors", credentials: "omit" });
        if (!response.ok) throw new Error(`${label}: HTTP ${response.status}`);
        const blob = await response.blob();
        if (!blob.size) throw new Error(`${label}: file kosong`);
        return blob;
    }

    // Video is required; optional assets are skipped if their media host blocks CORS.
    try {
        const blob = await fetchBlob(videoUrl, "Video");
        files.push({ name: "video.mp4", blob });
        setDownloadStatus(35, "Video siap...");
    } catch (error) {
        console.warn("Pack video gagal:", error);
        setDownloadStatus(0, "Video gagal diambil");
        showToast("Video tidak bisa dimasukkan ke ZIP. Coba Download MP4 biasa.", "error");
        return;
    }

    if (audioUrl) {
        try {
            const blob = await fetchBlob(audioUrl, "Audio");
            files.push({ name: "audio.mp3", blob });
            setDownloadStatus(55, "Audio siap...");
        } catch (error) {
            console.warn("Pack audio dilewati:", error);
        }
    }

    if (coverUrl) {
        try {
            const blob = await fetchBlob(coverUrl, "Thumbnail");
            const type = blob.type && blob.type.includes("png") ? "png" : "jpg";
            files.push({ name: `thumbnail.${type}`, blob });
            setDownloadStatus(70, "Thumbnail siap...");
        } catch (error) {
            console.warn("Pack thumbnail dilewati:", error);
        }
    }

    const caption = String(currentVideo.title || "").trim();
    const author = currentVideo.author?.unique_id || currentVideo.author?.uniqueId || "";
    const captionText = [
        caption || "(Caption tidak tersedia)",
        "",
        author ? `Creator: @${author}` : "",
        getVideoShareUrl() ? `Link: ${getVideoShareUrl()}` : ""
    ].filter(Boolean).join("\n");
    files.push({ name: "caption.txt", blob: new Blob([captionText], { type: "text/plain;charset=utf-8" }) });
    setDownloadStatus(82, "Caption siap...");

    try {
        const zip = new JSZip();
        files.forEach(file => zip.file(file.name, file.blob));
        const zipBlob = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } }, meta => {
            const percent = 82 + Math.round(meta.percent * 0.15);
            setDownloadStatus(percent, "Membuat ZIP...");
        });

        const objectUrl = URL.createObjectURL(zipBlob);
        const link = document.createElement("a");
        link.href = objectUrl;
        link.download = `${safeTitle} - TikGrab Pack.zip`;
        link.style.display = "none";
        document.body.appendChild(link);
        link.click();
        link.remove();
        ...
        setDownloadStatus(100, "Download Pack siap!");
        showToast(`Download Pack selesai (${files.length} file).`, "success");
    } catch (error) {
        console.error("Download Pack error:", error);
        setDownloadStatus(0, "Gagal membuat ZIP");
        showToast("Gagal membuat Download Pack. Coba lagi.", "error");
    }
}

/* TikGrab 5.2 — public rating & reviews via Supabase */
(function initPublicReviews(){
 const stars=[...document.querySelectorAll('#ratingStars [data-rating]')];
 const submit=document.getElementById('submitReview'), name=document.getElementById('reviewName'), text=document.getElementById('reviewText');
 const status=document.getElementById('reviewFormStatus'), list=document.getElementById('reviewsList'), empty=document.getElementById('noReviews');
 const avg=document.getElementById('reviewAverage'), count=document.getElementById('reviewCount');
 if(!stars.length || !submit || !name || !text || !status || !list || !empty || !avg || !count) return;
 let rating=0;
 const cfg=window.TIKGRAB_SUPABASE_CONFIG||{};
 const ready=!!(window.supabase && cfg.url && cfg.publishableKey && !cfg.url.includes('YOUR_') && !cfg.publishableKey.includes('YOUR_'));
 const client=ready ? window.supabase.createClient(cfg.url,cfg.publishableKey) : null;
 function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}
 function starsText(n){return '★★★★★'.slice(0,n)+'☆☆☆☆☆'.slice(0,5-n)}
 function update(){stars.forEach(b=>{const v=+b.dataset.rating;b.classList.toggle('text-yellow-400',v<=rating);b.classList.toggle('text-gray-300',v>rating)});submit.disabled=rating<1;status.textContent=rating?`Rating dipilih: ${rating}/5`:'Pilih rating terlebih dahulu.'}
 function render(data){
   const total=data.reduce((s,x)=>s+Number(x.rating||0),0);
   avg.textContent=data.length?`${(total/data.length).toFixed(1)} / 5`:'0.0 / 5';
   count.textContent=data.length?`${data.length} ulasan`:'Belum ada ulasan';
   list.innerHTML=''; empty.classList.toggle('hidden',data.length>0);
   data.forEach(x=>{const c=document.createElement('div');c.className='tg52-review-card border border-gray-100 rounded-xl p-4 bg-white';c.innerHTML=`<div class="flex items-start justify-between gap-3"><div><div class="font-semibold text-gray-900">${esc(x.name||'Pengguna TikGrab')}</div><div class="text-yellow-400 text-sm mt-0.5">${starsText(Number(x.rating))}</div></div><div class="text-[11px] text-gray-400">${esc(new Date(x.created_at||Date.now()).toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'}))}</div></div><p class="text-sm text-gray-600 mt-3 leading-6">${esc(x.review_text)}</p>`;list.appendChild(c)})
 }
 async function load(){
   if(!client){
     avg.textContent='—'; count.textContent='Backend belum terhubung'; empty.textContent='Rating publik akan aktif setelah Supabase dikonfigurasi.'; empty.classList.remove('hidden'); return;
   }
   const {data,error}=await client.from('reviews').select('id,rating,name,review_text,created_at').eq('status','published').order('created_at',{ascending:false}).limit(100);
   if(error){console.error(error);status.textContent='Rating belum dapat dimuat. Coba lagi nanti.';return;}
   render(data||[]);
 }
 stars.forEach(b=>b.addEventListener('click',()=>{rating=+b.dataset.rating;update()}));
 submit.addEventListener('click',async()=>{
   if(!rating)return;
   const t=text.value.trim(), n=name.value.trim().slice(0,40);
   if(!t){status.textContent='Tulis ulasan terlebih dahulu.';text.focus();return}
   if(!client){status.textContent='Backend rating belum dikonfigurasi.';return}

   const cooldownUntil=Number(localStorage.getItem('tgReviewCooldownUntil')||0);
   if(Date.now()<cooldownUntil){
     const seconds=Math.ceil((cooldownUntil-Date.now())/1000);
     status.textContent=`Coba lagi dalam ${seconds} detik.`;
     return;
   }
   submit.disabled=true; status.textContent='Mengirim ulasan...';
   const {error}=await client.rpc('submit_review', {
     p_rating: rating,
     p_name: (n || 'Pengguna TikGrab').slice(0,40),
     p_review_text: t.slice(0,300)
   });
   if(error){console.error(error);status.textContent='Ulasan gagal dikirim. Coba lagi.';submit.disabled=false;return}
   localStorage.setItem('tgReviewCooldownUntil', String(Date.now()+60000));
   rating=0;name.value='';text.value='';update();status.textContent='Ulasan berhasil dikirim. Terima kasih! ❤️';await load();
 });
 update();load();
})();


/* TikGrab 5.0 — isolated batch downloader */
(function initBatchDownloader(){
    ...
    if (!input || !countEl || !startBtn) return;
    ...
    function updateCount(){
        ...
        const urls = getUrls();
        countEl.textContent = `${urls.length} link terdeteksi` +
            (rawLines.length > MAX_BATCH ? ` • maksimal ${MAX_BATCH}` : '');
        startBtn.disabled = urls.length === 0;
    }

    function addResult(index, url, state, message){
        const row = document.createElement('div');
        row.className = 'flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3 text-sm';
        const icon = state === 'ok' ? '✅' : state === 'working' ? '⏳' : '❌';
        row.innerHTML = `
            <span class="text-base">${icon}</span>
            <div class="min-w-0 flex-1">
              <div class="font-medium text-gray-800">#${index + 1} ${escapeBatch(url)}</div>
              <div class="text-xs text-gray-500 mt-0.5">${escapeBatch(message)}</div>
            </div>`;
    ...
    /*
     * Uses the same API endpoint as the existing TikGrab downloader.
     * The batch flow intentionally fetches one item at a time to avoid
     * hammering the public API and to keep the UI predictable.
     */
    async function processOne(url, index){
        const row = addResult(index, url, 'working', 'Mengambil data video...');
        try {
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 20000);
            const response = await fetch(typeof API_URL !== 'undefined' ? API_URL : 'https://www.tikwm.com/api/', {
                method: 'POST',
            ...
            // Reuse the proven direct-download behavior, but with a unique filename.
            const ext = /\.mp4(?:\?|$)/i.test(videoUrl) ? 'mp4' : 'mp4';
            const username =
                (data.author && (data.author.unique_id || data.author.uniqueId || data.author.nickname)) ||
                (data.author_info && (data.author_info.unique_id || data.author_info.uniqueId || data.author_info.nickname)) ||
                'user';
            const videoId = data.id || data.aweme_id || data.video_id || `${Date.now()}-${index + 1}`;
            const safeUsername = String(username).replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 40) || 'user';
            const safeVideoId = String(videoId).replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 40);
            const filename = `TikGrab_@${safeUsername}_${safeVideoId}.${ext}`;

            try {
                const media = await fetch(videoUrl, {mode:'cors', credentials:'omit'});
                ...
                row.querySelector('.text-xs').textContent = 'Download dimulai.';
                return true;
            } catch (downloadError) {
                // Browser/server CORS restrictions can prevent blob download.
                // Fall back to the native download navigation.
                const a = document.createElement('a');
                a.href = videoUrl;
                a.download = filename;
                a.rel = 'noopener';
                ...
                row.querySelector('.text-xs').textContent = 'Data berhasil diproses; browser memakai metode download alternatif.';
                return true;
            }
        } catch (error) {
            row.querySelector('span').textContent = '❌';
            row.querySelector('.text-xs').textContent =
                error && error.name === 'AbortError' ? 'Timeout saat mengambil data.' :
                (error.message || 'Gagal memproses link.');
            return false;
        }
    }
    ...
    startBtn.addEventListener('click', async () => {
        ...
        for (let i = 0; i < urls.length; i++) {
            ...
            progressText.textContent = `Memproses ${i + 1} dari ${urls.length}...`;

            if (await processOne(urls[i], i)) success++;

            // Small pause between requests to keep the public API load reasonable.
            if (i < urls.length - 1) await wait(700);
        }

        progressPercent.textContent = '100%';
        progressBar.style.width = '100%';
        progressText.textContent = `Selesai • ${success}/${urls.length} berhasil`;
